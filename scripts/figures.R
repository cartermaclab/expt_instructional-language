...
#> QUESTIONNAIRES ----
#>
#> Create tibbles of questionnaires just before retention
pa_ret <- pa_p_tib %>%
  dplyr::filter(time_id == "t3") %>%
  dplyr::rename("pa" = "score") %>%
  dplyr::select(-c(3,4))
...
im_ret <- im_p_tib %>%
  dplyr::filter(time_id == "t3") %>%
  dplyr::rename("im" = "score") %>%
  dplyr::select(-c(3,4))

#> Change sub_id in motor data
adj_ret_p_tib_id <- adj_ret_p_tib
adj_ret_p_tib_id$sub_id <- sub("^", "S", adj_ret_p_tib_id$sub_id)

#> Merge all tibbles together
ret_tib <- adj_ret_p_tib_id %>%
  dplyr::left_join(pa_ret) %>%
  dplyr::left_join(pc_ret) %>%
  dplyr::left_join(im_ret) %>%
